...
const PRICE = "20000"; // 0.02 USDC, 6 decimals
...
function input(req) {
  if (req.method === "GET") {
    ...
    for (const [key] of CHECKS) {
      ...
      if (value === "true" || value === "1" || value === true) data[key] = true;
      else if (value === "false" || value === "0" || value === false) data[key] = false;
    }
    return data;
  }
...
function resourceUrl(req) {
  ...
  const host = header(req, "x-forwarded-host") || header(req, "host") || "www.dv9.com.ua";
...
function evaluate(data) {
  ...
  if (failed.length) {
    return {
      ...
      disclaimer: "Decision-support only. This is not approval to pour concrete and does not replace formal inspection or licensed engineering sign-off.",
    };
  }
  return {
    status: "READY_FOR_FORMAL_REVIEW",
    failed_checks: [],
    next_actions: ["Proceed to the project's required formal inspection / hidden-works acceptance workflow before any pour."],
    disclaimer: "READY_FOR_FORMAL_REVIEW is not permission to pour concrete. Formal acceptance remains required.",
  };
}
...
async function facilitator(path, paymentPayload, requirement) {
  ...
  catch { data = { success: false, isValid: false, errorReason: `facilitator_http_${response.status}` }; }
...
export default async function handler(req, res) {
  ...
  if (missing.length) return res.status(400).json({ error: "MISSING_REQUIRED_CHECKS", missing });

  const challenge = required(req);
  const requirement = challenge.accepts[0];
  const signatureHeader = header(req, "payment-signature");
  if (!signatureHeader) return paymentRequired(res, challenge, "PAYMENT-SIGNATURE header is required");

  let paymentPayload;
  try { paymentPayload = unb64(signatureHeader); }
  catch { return paymentRequired(res, challenge, "PAYMENT-SIGNATURE header is invalid"); }

  const a = paymentPayload?.accepted;
  if (
    paymentPayload?.x402Version !== 2 || !a ||
    a.scheme !== requirement.scheme || a.network !== requirement.network ||
  ...
  let verify;
  try { verify = await facilitator("/verify", paymentPayload, requirement); }
  catch { return res.status(503).json({ error: "PAYMENT_VERIFICATION_UNAVAILABLE" }); }
  if (!verify.ok || verify.data?.isValid !== true) {
    return paymentRequired(res, challenge, verify.data?.invalidReason || verify.data?.errorReason || "Payment verification failed");
  }

  const result = evaluate(data);

  let settle;
  try { settle = await facilitator("/settle", paymentPayload, requirement); }
  catch { return res.status(503).json({ error: "PAYMENT_SETTLEMENT_UNAVAILABLE" }); }
  if (!settle.ok || settle.data?.success !== true) {
    return paymentRequired(res, challenge, settle.data?.errorReason || "Payment settlement failed");
  }

  res.setHeader("PAYMENT-RESPONSE", b64(settle.data));
  return res.status(200).json({
    ...
    payment: {
      ...
      transaction: settle.data?.transaction || null,
    },
    result,
  });
}
